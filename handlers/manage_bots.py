"""
========================================================================
 МОДУЛЬ: «🗂 Управление ботами» + карточка бота.
------------------------------------------------------------------------
 Список ботов пользователя и страница конкретного бота (как на скринах):
 обновить статистику, перезапуск, защита, рассылка, статистика,
 изменить шаблон, настройки, удалить, назад.
========================================================================
"""
...
async def do_delete(callback: CallbackQuery) -> None:
    ...
    if not owns(callback.from_user.id, bot):
        await callback.answer("Бот не найден.", show_alert=True)
        return
    # Останавливаем polling и удаляем из БД.
    if bot.get("tg_id"):
