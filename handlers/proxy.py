...
========================================================================
 МОДУЛЬ: «🌐 Выбор прокси»
------------------------------------------------------------------------
 Пул прокси владельца (общий для всех его ботов) для поднятия дочерних
 ботов. Бот ссылается на выбранный прокси (bots.proxy_id); polling
 поднимается через него. После смены прокси бот перезапускается.

 Поддерживаемые форматы при добавлении (по одному в строке):
...
def _normalize(line: str) -> str | None:
    """Приводим строку прокси к URL со схемой. None — если непонятно."""
    line = line.strip()
    if not line:
        return None
    if "://" in line:
...
def _mask(url: str) -> str:
    """Прячем пароль в отображении прокси."""
...
async def set_proxy(callback: CallbackQuery) -> None:
    ...
    # перезапускаем бота, чтобы прокси применился
